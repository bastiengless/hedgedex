...
# Define model architecture and utils
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, n_channels, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(self.n_channels, 64, kernel_size=3, stride=1,
                               padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        # Removed because of the Random Crop
        # out_aux = F.interpolate(out_aux, size=(256, 256), mode='bilinear', align_corners=ALIGN_CORNERS)
        # out = F.interpolate(out, size=(256, 256), mode='bilinear', align_corners=ALIGN_CORNERS)
    ...
    def evaluate(self, valloader, metrics, device):
        ...
        with torch.no_grad():
            for batch in valloader:
                ...
                for i, metric in enumerate(metrics):
                    scores[i] += metric(outputs[1], labels).item()
        for i, metric in enumerate(metrics):
            scores[i] /= len(valloader)
        return scores

def get_seg_model(cfg, n_channels=3, pretrained=True, **kwargs):
    model = HighResolutionNet(cfg, n_channels, **kwargs)
    if pretrained:
