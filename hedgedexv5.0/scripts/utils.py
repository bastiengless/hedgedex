...
class TIFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with rasterio.open(img_path) as src:
            ...
            restricted_band_names = [band_names[i][:-3] for i in range(9)]

        # Restack the raster to have the bands in the correct order
        restacked_list = []
        for i in range(12):
            restacked_list.append(raster[i*9:(i+1)*9, :, :])
        restacked = np.stack(restacked_list, axis=1)

        # Make indices correspond to band_names
        indices = [restricted_band_names.index(band) for band in self.bands]

        # Only take selected bands
        image = restacked[indices, :, :, :]
        label = raster[-1, :, :]
