...
def train_model(model, trainloader, valloader, criterion, optimizer, metrics, n_epochs, model_save_path, metrics_save_path, curve_save_path, device):
    ...
    torch.save(model.state_dict(), 'hedgedexv5.0/output/models/last_terminal_model.pth')
...
def load_data(data_dir, n_bands, bs, dataset_size):
    # TODO : separate bands per month
    
    # TODO : then stack on a time dimension
...
def main():
    ...
    model_save_path = f'hedgedexv5.0/output/models/model_{hp_stamp}.pth'
    metrics_save_path = f'hedgedexv5.0/output/metrics/metrics_{hp_stamp}.csv'
    curve_save_path = f'hedgedexv5.0/output/plots/curve_{hp_stamp}.png'
    ...
    model = UNet_time(n_channels=n_bands, n_classes=1).to(device)
